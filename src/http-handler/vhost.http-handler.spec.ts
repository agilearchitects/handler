...
describe("vhostHttpHandler", () => {
  it("should validate host using url", async () => {
    ...
    const response = await basicHandle(Method.GET, "http://www.test.test", {}, null,
  ...
  it("should validate host using header before url", async () => {
    const message: string = "hello world";
    const response = await basicHandle(Method.GET, "http://www.test.test", { host: "api.test.test"}, null,
  ...
  it("should validate using array", async () => {
    ...
    const response1 = await basicHandle(Method.GET, "http://www.test.test", {}, null, ...handlers);
    const response2 = await basicHandle(Method.GET, "http://api.test.test", {}, null, ...handlers);
    const response3 = await basicHandle(Method.GET, "http://wrong.test.test", {}, null, ...handlers);
