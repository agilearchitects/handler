...
import { handlerMethod, Method, handle, headers, header, response, HttpHandlerError, HttpHandler } from "../http-handler";
...
export type lambdaHandlerMethod = (event: IAPIGatewayProxyEvent) => Promise<IAPIGatewayProxyResult>;
...
export function lambdaHandler(options: ILambdaOptions, ...handlers: handlerMethod[]): lambdaHandlerMethod;
export function lambdaHandler(...handlers: handlerMethod[]): lambdaHandlerMethod;
export function lambdaHandler(options: ILambdaOptions | handlerMethod, ...handlers: handlerMethod[]): lambdaHandlerMethod {
  if(typeof options === "function") {
    handlers = [options, ...handlers];
    options = {};
  }

  return async (event: IAPIGatewayProxyEvent): Promise<IAPIGatewayProxyResult> => {
    ...
    try {
      handleResponse = await handle(method, url, headers, event.body, ...handlers);
    } catch(error: unknown) {
